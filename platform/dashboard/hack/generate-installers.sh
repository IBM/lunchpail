...
# lite deployment
helm template --include-crds codeflare-platform "$TOP"/platform $HELM_DEMO_SECRETS --set global.lite=true --set tags.examples=false --set tags.defaults=false --set tags.full=false --set tags.core=true \
     2> >(grep -v 'found symbolic link' >&2) \
     2> >(grep -v 'Contents of linked' >&2) \
...
# defaults
helm template --include-crds codeflare-platform "$TOP"/platform --set tags.examples=false --set tags.defaults=true --set tags.full=false --set tags.core=false \
     2> >(grep -v 'found symbolic link' >&2) \
     2> >(grep -v 'Contents of linked' >&2) \
     > "$OUTDIR"/jaas-defaults.yml

# examples
helm template --include-crds codeflare-platform "$TOP"/platform $HELM_DEMO_SECRETS --set tags.examples=true --set tags.defaults=false --set tags.full=false --set tags.core=false \
     2> >(grep -v 'found symbolic link' >&2) \
     2> >(grep -v 'Contents of linked' >&2) \
