...
(while true; do kubectl wait pod -l app.kubernetes.io/instance=$run_id,app.kubernetes.io/component=kubeflow --for=condition=Ready --timeout=-1s -n $namespace >& /dev/null && break; sleep 1; done) &

helm install --dry-run --debug $run_id "$SCRIPTDIR" -n ${namespace} \
     --set uid=$uid \
     --set name=$name \
     --set image=$image \
...
# script!
while true; do
    HEAD=$(kubectl get pod -n $namespace -l app.kubernetes.io/instance=$run_id,app.kubernetes.io/component=kubeflow --no-headers -o custom-columns=NAME:.metadata.name)
    if [[ -n "$HEAD" ]]; then
        echo -n $HEAD
        break
    fi
