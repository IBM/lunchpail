...
def create_run(name: str, namespace: str, uid: str, spec, patch, **kwargs):
    ...
    try:
        ...
        if api == "ray":
            create_run_ray(v1Api, application, namespace, uid, name, spec, command_line_options, run_size_config, patch)
        elif api == "torch":
            create_run_torch(v1Api, application, namespace, uid, name, spec, command_line_options, run_size_config, patch)
        else:
