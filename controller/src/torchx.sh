...
torchx run --dryrun \
    ...
    | sed -E "s#^([ \t]*)app.kubernetes.io/managed-by: torchx.pytorch.org#\1app.kubernetes.io/managed-by: lunchpail.io\n\1app.kubernetes.io/part-of: ${part_of}\n\1app.kubernetes.io/step: '${step}'#" \
    ...
    | awk -v workdirPVC=$WORKDIR_PVC '{ idx=index($0, "volumes:"); print $0; if (idx > 0) { for (i=1; i<idx; i++) printf " "; print "- name: workdir-volume"; for (i=1; i<idx+2; i++) printf " "; print "persistentVolumeClaim:"; for (i=1; i<idx+4; i++) printf " "; print "claimName:", workdirPVC; } }' \
          > $DRY
