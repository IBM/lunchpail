...
func upLLIR(ctx context.Context, backend be.Backend, ir llir.LLIR, opts UpOptions) error {
	...
	go func() {
		...
		case ctx := <-isRunning6:
			if ctx.Run.Step == 0 || isFinalStep(ctx) {
				errorFromAllDone = waitForAllDone(cancellable, backend, ctx.Run, ctx.Queue, *opts.BuildOptions.Log)
	...
	go func() {
		...
		if err := lookForTaskFailures(cancellable, backend, ir.Context.Run, ir.Context.Queue, *opts.BuildOptions.Log); err != nil {
			errorFromTask = err
			// fail fast? cancel()
		}
	}()
	...
	if opts.Executable != "" {
		go func() {
			...
			if opts.BuildOptions.Target.Platform == target.IBMCloud {
				...
				if err := s3.UploadFiles(cancellable, backend, ir.Context.Run, []upload.Upload{upload.Upload{LocalPath: opts.Executable + "-linux-amd64", TargetDir: ir.Context.Run.AsFile(q.Blobs)}}, ir.Context.Queue, *opts.BuildOptions.Log); err != nil {
					fmt.Fprintln(os.Stderr, err)
				}
				...
				if err := s3.UploadFiles(cancellable, backend, ir.Context.Run, []upload.Upload{upload.Upload{LocalPath: opts.Executable, TargetDir: ir.Context.Run.AsFile(q.Blobs)}}, ir.Context.Queue, *opts.BuildOptions.Log); err != nil {
					fmt.Fprintln(os.Stderr, err)
				}
			}
		}()
