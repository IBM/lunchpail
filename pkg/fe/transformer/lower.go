...
func Lower(buildName string, model hlir.HLIR, ctx llir.Context, opts build.Options) (llir.LLIR, error) {
	...
	minio, err := minio.Lower(buildName, ctx.Run, model, ir, opts)
	if err != nil {
	...
	apps, err := lowerApplications(buildName, ctx.Run, model, ir, opts)
	if err != nil {
		return llir.LLIR{}, err
	}

	dispatchers, err := dispatch.Lower(buildName, ctx.Run, model, ir, opts)
	if err != nil {
		return llir.LLIR{}, err
	}

	pools, err := workerpool.LowerAll(buildName, ctx.Run, model, ir, opts)
	if err != nil {
		return llir.LLIR{}, err
	}
