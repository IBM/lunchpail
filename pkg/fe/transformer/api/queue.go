...
const (
	Unassigned            = "lunchpail/run/{{.RunName}}/step/{{.Step}}/unassigned/{{.Task}}"
...
func (q PathArgs) TemplateDir(path string) string {
...
func (q PathArgs) PatternFor(s string) *regexp.Regexp {
	s = q.ForPool(placeholder).ForWorker(placeholder).ForTask(placeholder).TemplateP(s)
	return regexp.MustCompile(placeholderR.ReplaceAllString(s, "(.+)"))
}
...
// file.
func (q PathArgs) ForObject(path, object string) (PathArgs, error) {
