...
func createSSHKey(vpcService *vpcv1.VpcV1, name string, resourceGroupID string, keyType string, pubKey string) (string, error) {
	...
	if err != nil {
		if (response.StatusCode == http.StatusBadRequest || response.StatusCode == http.StatusConflict) && err.Error() == "Key with fingerprint already exists" {
...
func createResources(ctx context.Context, vpcService *vpcv1.VpcV1, name string, ir llir.LLIR, resourceGroupID string, keyType string, publicKey string, zone string, profile string, imageID string, namespace string, opts llir.Options) (string, error) {
	...
	t6s := time.Now()
	if err = createVMForComponents(ctx, vpcService, name, ir, resourceGroupID, zone, profile, imageID, namespace, vpcID, keyID, subnetID, secGroupID, opts, t6s); err != nil {
		return "", err
	}
	t6e := time.Now()
...
func createVMForComponents(ctx context.Context, vpcService *vpcv1.VpcV1, name string, ir llir.LLIR, resourceGroupID string, zone string, profile string, imageID string, namespace string, vpcID string, keyID string, subnetID string, secGroupID string, opts llir.Options, provTime time.Time) error {
	...
	for _, c := range ir.Components {
		...
		componentB64, err := util.ToJsonGzipB64(c)
		if err != nil {
			fmt.Fprintf(os.Stderr, "%v\n", err)
			return err
		}

		llirB64, err := util.ToJsonGzipB64(ir)
		if err != nil {
			fmt.Fprintf(os.Stderr, "%v\n", err)
			return err
		}
		...
		cc := &config.CloudConfig{
			RunCmd: []string{"s=$(date +%s)",
				...
				"mc get myminio/" + ir.Context.Queue.Bucket + "/" + ir.Context.Run.AsFile(q.Blobs) + "/$exec /lunchpail",
				"chmod +x /lunchpail",
				"s=$(date +%s)",
				"env HOME=/root /lunchpail component run-locally --component " + componentB64 + " --llir " + llirB64 + " " + verboseFlag,
				"e=$(date +%s)",
				"echo \"METRICS: Took $(($e-$s)) seconds for running LP component\""},
		}
		...
		group.Go(func() error {
			...
			t6e := time.Now()
			if opts.Log.Verbose {
				fmt.Fprintf(os.Stderr, "  - VSI for %s took %s\n", instanceName, util.RelTime(t6s, t6e))
			}
			...
			t7e := time.Now()
			if opts.Log.Verbose {
				fmt.Fprintf(os.Stderr, "  - FP IP for %s took %s\n", instanceName, util.RelTime(t7s, t7e))
			}
			return nil
		})
	}
