...
func message(who, message string, style lipgloss.Style) string {
	return fmt.Sprintf("%s %s", style.Render(fmt.Sprintf("%8s", who)), colors.Dim.Render(message))
}
...
func cpuline(workers []cpu.Worker, resource Resource) string {
	...
	return message(string(resource), strings.Join(line, ""), colors.OtherComponentStyle)
}
...
func footer(model Model, timestamp time.Time, maxheight int) []string {
	...
	for _, msg := range model.messages(maxheight) {
		footer = append(footer, message(msg.who, msg.message, colors.DispatcherComponentStyle))
	}
