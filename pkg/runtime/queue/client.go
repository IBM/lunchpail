...
func NewS3ClientForRun(ctx context.Context, backend be.Backend, run queue.RunContext, rclone string, opts build.LogOptions) (S3ClientStop, error) {
	endpoint, accessKeyId, secretAccessKey, bucket, stop, err := backend.AccessQueue(ctx, run, rclone, opts)
	if err != nil {
