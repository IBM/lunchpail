...
func UploadFiles(ctx context.Context, backend be.Backend, run queue.RunContext, specs []upload.Upload, queue queue.Spec, opts build.LogOptions) error {
	s3, err := NewS3ClientForRun(ctx, backend, run, queue, opts)
	if err != nil {
		return err
	}
	defer s3.Stop()
