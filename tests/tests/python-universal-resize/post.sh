...
DATA="$TEST_PATH"/pail/test-data/ds01
...
# TODO until we handle multiple outputs for a single input, we only validate that *some* output was produced
validate task.1.output.txt #"$DATA"/expected/lang=en/pq01.parquet.gz
validate task.2.output.txt #"$DATA"/expected/lang=en/pq02.parquet.gz
