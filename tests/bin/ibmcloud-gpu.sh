...
if [[ -n $apikey ]] && [[ -n $resource_group ]] && [[ -n $vpc_id ]] && [[ -n $ssh_key_id ]] && [[ -n $subnet_id ]] && [[ -n $security_group_id ]] && [[ -n $ssh_key ]]; then
    ...
    then
        ...
        ssh -t -t -i /tmp/.ssh/gpu_ssh.prv root@$ip "function cleanup { export apikey=$apikey; ~/cfp/hack/ibmcloud/delete_vm.sh '${GPU_CONFIG}' >& /tmp/delete_vm.out; exit 0; }; trap cleanup EXIT; export CI=$CI; export DEBUG=$DEBUG; export NO_KUBEFLOW=1; sudo sysctl fs.inotify.max_user_instances=8192; cd cfp; ./tests/bin/test.sh; code=$?; echo \"Remote gpu tests finished with code=$code (this message is from the remote host)\"; trap - EXIT; exit $code"
        code=$?
